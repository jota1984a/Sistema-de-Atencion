...
/* Created on:     19/01/2017 22:36:26                          */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('PRIVILEGIO') and o.name = 'FK_PRIVILEG_INTEGRA_ROL')
alter table PRIVILEGIO
   drop constraint FK_PRIVILEG_INTEGRA_ROL
go
...
if exists (select 1
           ...
           where  id    = object_id('PRIVILEGIO')
            ...
            and   indid > 0
            and   indid < 255)
   drop index PRIVILEGIO.INTEGRA_FK
go
...
create nonclustered index INTEGRA_FK on PRIVILEGIO (ID_ROL ASC)
go
...
create table SUCURSAL (
   ...
   LATITUD_SUCURSAL     varchar(20)          not null,
   LONGITUD_SUCURSAL    varchar(20)          not null,
...
alter table PRIVILEGIO
   add constraint FK_PRIVILEG_INTEGRA_ROL foreign key (ID_ROL)
      references ROL (ID_ROL)
go
